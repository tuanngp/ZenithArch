...
| --- | --- |
| Dùng CQRS hoặc FullStack cơ bản | `AddZenithArchDependencies()` |
| Dùng Repository/FullStack có `UseUnitOfWork = true` | `AddZenithArchDependencies<AppDbContext>()` |
| Muốn tự cấu hình MediatR | `AddZenithArchDependencies(registerMediatR: false)` |
...
Ghi chú:
- `AddZenithArchDependencies()` có thể tự động đăng ký MediatR theo mặc định.
- Nếu bạn đã có cấu hình MediatR riêng, dùng `AddZenithArchDependencies(registerMediatR: false)`.
...
Validation behavior:
- Khi `EnableValidation = true`, generated DI sẽ đăng ký `ZenithArchValidationBehavior<,>`.
...
- Generated handlers và validation behavior resolve `IZenithArchExecutionObserver` dưới dạng `IEnumerable<T>`, nên có thể đăng ký 0, 1 hoặc nhiều observer.
...
Ghi chú:
- Khi `UseUnitOfWork = true`, ưu tiên `AddZenithArchDependencies<TDbContext>()` để auto-wire generated `IUnitOfWork`.
...
- gọi `AddZenithArchDependencies()` để wire generated cache behaviors và invalidators
