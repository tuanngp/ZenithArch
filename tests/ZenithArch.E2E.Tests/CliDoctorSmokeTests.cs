...
namespace ZenithArch.E2E.Tests;
...
public sealed class CliDoctorSmokeTests
{
    ...
    {
        string repoRoot = FindRepoRoot();
        string samplePath = Path.Combine(repoRoot, "samples", "ZenithArch.Sample");
    ...
    {
        string cliProject = Path.Combine(repoRoot, "src", "ZenithArch.Cli", "ZenithArch.Cli.csproj");
    ...
    {
        string path = Path.Combine(Path.GetTempPath(), "zenitharch-e2e-" + Guid.NewGuid().ToString("N"));
        ...
        File.WriteAllText(Path.Combine(path, "Trip.cs"),
            "using ZenithArch.Abstractions.Attributes; [Entity] public class Trip { public string Name { get; set; } = string.Empty; }");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(current, "ZenithArch.slnx")))
            {
                return current;
            }
